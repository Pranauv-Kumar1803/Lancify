...
router.get('/order/:id',async(req,res)=>{
	...
	if(user.user_type === 'seller') {
		console.log('in');
		return res.render('pages/viewOrderSeller',{order,login:true, pending: order.pending})
	}
	else {
		return res.render('pages/viewOrder',{order,login:true, pending: order.pending, rated: order.rating})
	}
})
...
router.post('/order/:id/addTimeline',async(req,res)=>{
	...
	try {
		...
		console.log('in');
		if(files.length > 0) {
			order.timeline.push({title : `${user.username} : ${title}`,message,date:d, files: files});
		}
		else {
			order.timeline.push({ title : `${user.username} : ${title}`, message, date:d });
		}
...
router.get("/profile", async (req, res) => {
	...
	if(user.user_type === 'seller') {
		...
		const seller_orders = await Order.find({seller_id: req.user, pending: false}).populate('service_id');
		const ongoing = await Order.find({seller_id: req.user, pending: true}).populate('service_id');
		return res.render("pages/dash", { user, seller, gigs, orders: seller_orders, ongoing });
	}

	const orders = await Order.find({user_id: req.user, pending: false}).populate('service_id');
	const ongoing = await Order.find({user_id: req.user, pending: true}).populate('service_id');
	console.log(orders);
...
router.post('/user/update',async(req,res)=>{
	...
	if(obj.password.length == 0) {
		...
		user.user_img = obj.image
		await user.save();
	}
	else
	{
		if(obj.password.length < 7) {
			return res.status(400).json({msg: 'password should atleast be 7 characters long!'});
		}
...
router.get("/product/:id", async(req, res) => {
	...
	if(check) {
		...
		{
			console.log('inside');
			return res.render('pages/product',{login: true, service, seller, check})
		}
		else {
			console.log('in');
	...
	{
		console.log('outside');
		return res.render('pages/product',{login: true, service, seller, check:undefined,key:process.env.STRIPE_PUBLISHABLE_KEY})
	}
});
...
router.post('/update_seller', async (req, res) => {
	try {
		...
		const res2 = await User.findOneAndUpdate({ user_id: req.user }, { $set: { user_img: obj.profile } }).exec();
